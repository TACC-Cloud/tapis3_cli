...
class SKCommonArgs(ParserExtender):
    def extend_parser(self, parser):
        ...
        parser.add_argument(
            "--secret-user", type=str, dest="secret_user", help="User [Tapis.user]"
        )
        return parser
    ...
    def process_secret_common_args(self, parsed_args):
        ...
        if self.secret_user is None:
            self.secret_user = self.tapis3_client.authenticator.get_userinfo().get(
                "username"
            )
...
class SKSecretNameArg(ParserExtender):
    def extend_parser(self, parser):
        parser.add_argument(
            "secret_name", metavar="SECRET_NAME", type=str, help="Secret Name"
        )
        return parser
