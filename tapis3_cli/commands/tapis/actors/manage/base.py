...
class ActorsManage(Oauth2FormatOne, StringIdentifier):
    ...
    def get_parser(self, prog_name):
        parser = super(ActorsManage, self).get_parser(prog_name)
    ...
    def extend_parser(self, parser):
        ...
        if self.CREATE_ONLY:
            super().add_identifier(
                parser, name="Actor Name", destination="actor_name", optional=False
            )
            parser.add_argument(
                "-d",
                "--description",
        ...
        parser.add_argument(
            "--webhook", metavar="URL", type=str, help="URL for event notifications"
        )
    ...
    def take_action(self, parsed_args):
        setattr(self, "config", self.get_configuration(parsed_args))
