...
class NoncesShow(Oauth2FormatOne, StringIdentifier):
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.is_alias:
            ...
            resp = self.tapis3_client.actors.getNonce(actor_id=actor_id,
                                                      nonce_id=nonce_id)
        ...
        if parsed_args.show_url:
            headers.append('url')
            data.append(
                self.tapis3_client.base_url + '/v3/actors/' + \
                actor_id + '/messages?x-nonce=' + nonce_id)
